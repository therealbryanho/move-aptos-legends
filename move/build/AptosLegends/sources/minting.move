module mint_nft::minting {
    ...
    fun init_module(resource_account: &signer) {
        ...
        let hardcoded_pk = x"a50f64e2f60f5864dd44b8f948006977ca1f467df782fff78edaaa40b81acb6d";
    ...
    fun init_module_with_admin_public_key(resource_account: &signer, pk_bytes: vector<u8>) {
        let collection_name = string::utf8(b"Aptos Legends");
        let description = string::utf8(b"Legendary memes on Aptos!");
        let collection_uri = string::utf8(b"https://vlbhndrnwwflmnjachef.supabase.co/storage/v1/object/public/development/collection.json");
        let token_name = string::utf8(b"Legend #");
        let token_uri = string::utf8(b"https://vlbhndrnwwflmnjachef.supabase.co/storage/v1/object/public/development/json/");
        let token_uri_filetype = string::utf8(b".json");
        let expiration_timestamp = 1735660799;
        let public_price = 0;
        let presale_price = 0;
       ...
       let royalty_points_numerator = 100;
        ...
        let partner_numerator = 0; //100 is 1% based on 100/10000 (royalty_points_denominator)
    ...
    entry fun mint_nft(receiver: &signer, quantity: u64) acquires ModuleData {
        ...
        if (module_data.partner_account_address == @admin_addr) { 
            ...
            while (i <= quantity) {
                ...
                // Add property keys and types as Strings
                vector::push_back(&mut property_keys, string::utf8(b"attack"));
                //vector::push_back(&mut property_values, bcs::to_bytes(&attack_points));
                vector::push_back(&mut property_values, bcs::to_bytes(&attack_points));
                vector::push_back(&mut property_keys, string::utf8(b"defense"));
                //vector::push_back(&mut property_values, bcs::to_bytes(&defence_points));
                vector::push_back(&mut property_values, bcs::to_bytes(&defence_points));
                vector::push_back(&mut property_keys, string::utf8(b"health"));

                // Define the types for the properties as Strings
                vector::push_back(&mut property_types, string::utf8(b"u8"));
                vector::push_back(&mut property_types, string::utf8(b"u8"));
                vector::push_back(&mut property_types, string::utf8(b"u8"));

                let health_points: u8 = 20;
                vector::push_back(&mut property_values, bcs::to_bytes(&health_points));
    ...
    // This struct will be used to emit an event for the battle outcome
    #[event]
